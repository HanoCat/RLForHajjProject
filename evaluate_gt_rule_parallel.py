import os
import random
import traceback
from concurrent.futures import ProcessPoolExecutor, as_completed
import multiprocessing as mp

import numpy as np
import pandas as pd
from shapely import wkt

from scenario_config import SCENARIO
from simulation_utils import *
from RL_utils import *
from barrier_control import clean_geom


OUTPUT_DIR = "outputs_parallel"
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "evaluation_gt_rule_parallel.csv")

SEEDS = [101, 202, 303, 404, 505]
NUM_AGENTS_LIST = [300, 500, 700, 900]
METHODS = ["gt_original", "rule_based_pilgrimage"]

NUM_EVAL_WORKERS = 10

_WORKER_ENV = None
_WORKER_AGENT_GROUPS = None


def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)


def get_original_gt_geometry(env):
    # Prefer simulation_geometry if available, otherwise connected_geometry.
    if "simulation_geometry" in env["geometry_wkt"]:
        return clean_geom(wkt.loads(env["geometry_wkt"]["simulation_geometry"]))
    return clean_geom(wkt.loads(env["geometry_wkt"]["connected_geometry"]))


def get_rule_based_states(num_agents):
    """
    Pilgrimage-inspired adaptive partial-opening rule:
    keep barriers partially open, increasing opening as crowd load increases,
    but avoid the fully-open unconstrained case.
    """
    if num_agents <= 300:
        opening = 0.45
    elif num_agents <= 500:
        opening = 0.60
    elif num_agents <= 700:
        opening = 0.70
    else:
        opening = 0.80

    return {
        pair_name: opening
        for pair_name in SCENARIO["barrier_pairs"].keys()
    }


def init_worker(env, agent_groups):
    global _WORKER_ENV, _WORKER_AGENT_GROUPS
    _WORKER_ENV = env
    _WORKER_AGENT_GROUPS = agent_groups


def run_eval_job(job):
    try:
        method = job["method"]
        seed = job["seed"]
        num_agents = job["num_agents"]

        set_seed(seed)

        pair_names = list(SCENARIO["barrier_pairs"].keys())

        if method == "gt_original":
            geometry = get_original_gt_geometry(_WORKER_ENV)
            barrier_pair_states = {p: np.nan for p in pair_names}

        elif method == "rule_based_pilgrimage":
            barrier_pair_states = get_rule_based_states(num_agents)
            geometry = create_geometry(_WORKER_ENV, barrier_pair_states)

        else:
            raise ValueError(f"Unknown method: {method}")

        sim_param = dict(SCENARIO["simulation_mode_training"])
        sim_param["write_trajectory"] = True
        sim_param["save_animation"] = False

        traj_dir = os.path.join(OUTPUT_DIR, "trajectories_gt_rule")
        os.makedirs(traj_dir, exist_ok=True)

        trajectory_file = os.path.join(
            traj_dir,
            f"{method}_seed{seed}_agents{num_agents}_pid{os.getpid()}.sqlite",
        )

        simulation = create_simulation(
            geometry,
            trajectory_file,
            sim_param,
        )

        set_seed(seed)

        selected_groups = select_agent_subset(
            _WORKER_AGENT_GROUPS,
            max_agents=num_agents,
            shuffle=True,
        )

        added, skipped = add_valid_agents_to_simulation(
            simulation,
            selected_groups,
            geometry,
        )

        result = run_simulation(
            simulation,
            SCENARIO["max_iterations"],
        )

        reward, metrics = compute_reward(
            result,
            trajectory_file=trajectory_file,
            geometry=geometry,
            debug=False,
        )

        row = {
            "method": method,
            "seed": seed,
            "num_agents_requested": num_agents,
            "added_agents": added,
            "skipped_agents": skipped,
            "reward": reward,
            "raw_reward": metrics["raw_reward"],
            "raw_cost": metrics["raw_cost"],
            "remaining_agents": result["remaining_agents"],
            "iterations": result["iterations"],
            "elapsed_time": result["elapsed_time"],
            "evacuation_ratio": metrics["evacuation_ratio"],
            "throughput_agents_per_second": metrics["throughput_agents_per_second"],
            "mean_speed": metrics["mean_speed"],
            "speed_loss": metrics["speed_loss"],
            "stopped_ratio": metrics["stopped_ratio"],
            "mean_density": metrics["voronoi_mean_density"],
            "max_density": metrics["voronoi_max_density"],
        }

        for p in pair_names:
            row[p] = barrier_pair_states[p]

        return {"ok": True, "row": row}

    except Exception as e:
        return {
            "ok": False,
            "job": job,
            "error": repr(e),
            "traceback": traceback.format_exc(),
        }


def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    _, env = load_environment(SCENARIO["env_json"])

    # Load agents using original GT geometry so GT and rule-based use the same candidate pool.
    base_geometry = get_original_gt_geometry(env)
    agent_groups = load_agents(base_geometry)

    jobs = []
    for seed in SEEDS:
        for num_agents in NUM_AGENTS_LIST:
            for method in METHODS:
                jobs.append({
                    "seed": seed,
                    "num_agents": num_agents,
                    "method": method,
                })

    print(f"Total GT/rule evaluation jobs: {len(jobs)}")
    print(f"Evaluation workers: {NUM_EVAL_WORKERS}")
    print(f"Output CSV: {OUTPUT_CSV}")

    rows = []
    ctx = mp.get_context("spawn")

    with ProcessPoolExecutor(
        max_workers=NUM_EVAL_WORKERS,
        mp_context=ctx,
        initializer=init_worker,
        initargs=(env, agent_groups),
    ) as executor:

        futures = [executor.submit(run_eval_job, job) for job in jobs]

        for i, future in enumerate(as_completed(futures), start=1):
            result = future.result()

            if result["ok"]:
                row = result["row"]
                rows.append(row)

                print(
                    f"[{i}/{len(jobs)}] done | "
                    f"{row['method']} | seed={row['seed']} | "
                    f"agents={row['num_agents_requested']} | "
                    f"raw_reward={row['raw_reward']:.3f} | "
                    f"remaining={row['remaining_agents']}"
                )

                pd.DataFrame(rows).to_csv(OUTPUT_CSV, index=False)

            else:
                print(f"[{i}/{len(jobs)}] FAILED")
                print(result["job"])
                print(result["error"])
                print(result["traceback"])

    df = pd.DataFrame(rows)
    df = df.sort_values(["seed", "num_agents_requested", "method"])
    df.to_csv(OUTPUT_CSV, index=False)

    print("\nSaved GT/rule evaluation to:", OUTPUT_CSV)

    print("\nMean results:")
    print(
        df.groupby("method")[
            [
                "reward",
                "raw_reward",
                "evacuation_ratio",
                "throughput_agents_per_second",
                "mean_speed",
                "stopped_ratio",
                "remaining_agents",
                "mean_density",
                "max_density",
            ]
        ].mean()
    )


if __name__ == "__main__":
    main()